// Functions to get the specific weather data
...
document.getElementById("btn-search").addEventListener("click", function () {
    ...
    fetch("https://api.openweathermap.org/data/2.5/forecast?q=" + city + "&units=metric&appid=403c880bbfd408e9697347abbaba5b73")

        .then(function (response) {
            if (response.ok === false) {     // fail to fetch => error. go to .catch part
                throw('Invalid city name :( Please check your city name again :) ');
            }
        ...
        .then(data => {
            ...
            /*future.forEach(function (item, i) {
                console.log(i+1);
                document.getElementsByClassName("date")[i+1].innerHTML = date[i+1];
                document.getElementsByClassName("icon")[i+1].src = "http://openweathermap.org/img/wn/" + getModeVal(item, "icon") + "d" + "@2x.png";
                document.getElementsByClassName("icon")[i+1].alt = getModeVal(item, "description")+" icon";
                document.getElementsByClassName("description")[i+1].innerHTML = getModeVal(item, "description");
                document.getElementsByClassName("temp")[i+1].innerHTML = getAvgTemp(item);
                document.getElementsByClassName("temp-min")[i+1].innerHTML = getMinTemp(item);
                document.getElementsByClassName("temp-max")[i+1].innerHTML = getMaxTemp(item);
                document.getElementsByClassName("humidity")[i+1].innerHTML = getAvgHumidity(item);
            });*/
            ...
            document.getElementsByClassName("temp")[0].innerHTML = Math.round(data.list[0].main.temp);
            document.getElementsByClassName("temp-min")[0].innerHTML = data.list[0].main.temp_min;
            document.getElementsByClassName("temp-max")[0].innerHTML = data.list[0].main.temp_max;
